...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/auth/twitch", (req, res) => {
    try {
      ...
      const redirectUri = config.twitch.redirectUri;

      // Ensure we have a fallback redirect URI
      const effectiveRedirectUri = redirectUri || `${req.protocol}://${req.get('host')}/api/auth/twitch/callback`;
      ...
      const authUrl = `https://id.twitch.tv/oauth2/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(effectiveRedirectUri)}&response_type=code&scope=${encodeURIComponent(scope)}&state=${state}`;
